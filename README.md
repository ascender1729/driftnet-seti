...
- **v1.1.0 (2026-09-20, revision for AJ referee report).** Fixed a fairness bug: the classical thresholded de-Doppler baseline in `baselines.py` searched 9 trial drifts on [-1, +1] channels per sample while the DriftNet variants searched 17 on [-3, +3]; the injected drifts of +-0.2 / +-0.4 Hz/s are +-1.31 / +-2.61 channels per sample, so the old baseline could not reach them. The baseline now uses the identical 17-point grid. Table 1 was re-run (classical row: in-distribution accuracy 0.720 -> 0.907, OOD 0.663 -> 0.822; learned rows unchanged within 0.005). `driftnet_comprehensive.py` now records the trained drift kernels per seed and the classical grid in the results JSON. `make_results_fig.py` draws majority-class chance (0.5 for the 2:1:1 injection set) instead of 1/3. `make_figures.py::concept` places the matched-filter marker on the injected line (the old marker sat on a noise peak) and adds axis ticks and labels. Second fix: `voyager_control.py` and `voyager_realsignal.py` had located the "carrier" as the argmax of the time-median spectrum, which is the coarse channel's central DC spike (channel 2^19, 8419.922 MHz), not the Voyager-1 carrier (8419.543 MHz, drifting -0.36 Hz/s per the turboSETI hit table). They now take the carrier channel and drift from the `.dat` next to the `.h5`, search +-3 ch/samp, and blank along the track; the corrected controls show DriftNet scoring the real carrier as SIGNAL (p = 1.000) and the curved detector fitting it with curvature 1.7e-5 Hz/s^2 inside the manifold. Re-run results are in `results/`.
